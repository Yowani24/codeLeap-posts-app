...
export const PostCard = ({ data, currentloggedInUser }: PostCardProps) => {
  const isAuthor = data.username === currentloggedInUser.toLowerCase();
  ...
  return (
    <article className="group relative bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-all duration-500 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] hover:-translate-y-1 overflow-hidden">
      <header className="relative h-20 bg-[#7695EC] flex items-center justify-between px-8 text-white overflow-hidden">
        ...
        <h2 className="relative z-10 font-black text-xl tracking-tight truncate max-w-[70%]">
          {data.title}
        </h2>
        ...
        {isAuthor && (
          <div className="relative z-10 flex items-center gap-2">
            <button
              className="p-2.5 bg-white/10 hover:bg-red-500/80 hover:scale-110 rounded-xl transition-all duration-300 backdrop-blur-sm border border-white/10"
              aria-label="Excluir post"
            >
              <FiTrash2 size={18} />
            </button>
            <button
              className="p-2.5 bg-white/10 hover:bg-white/20 hover:scale-110 rounded-xl transition-all duration-300 backdrop-blur-sm border border-white/10"
